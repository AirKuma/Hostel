...
<h4><span class="glyphicon glyphicon-list-alt"></span> 預約清單</h4>
</div>
<div class="panel-body">
    	<table class="table table-hover">
            <thead>
            <tr>
			<% for (int i=0; i<DBorder.TITLES.length; i++){ %>
			<th><%=DBorder.TITLES[i]%></th>
			<%} %>
			</tr>
            </thead>
            <tbody>
            <%
				...
				List<Order> orders = db.getAllOrder((String)session.getAttribute("user"));
				for (Order or: orders){
			%>
			<tr>
				<% for (int i=0; i<DBorder.TITLES.length; i++){ %>
