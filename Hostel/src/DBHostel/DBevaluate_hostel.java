...
public class DBevaluate_hostel extends DBcore {
	...
	public boolean isEvaluateHostel(String hostel_name, String user_email) throws Exception {
		...
		try {
			conn = makeConnection();
			st = conn.prepareStatement("SELECT * FROM Evaluate_hostel where user_email=? and hostel_name=?");
	...
	public List<Hostel> getEvaHostel(String user_email) throws Exception {
		...
		try {
			...
			st = conn.prepareStatement(
					"SELECT distinct hostel_name FROM BedandBreakfast,Booking,Room where Booking.user_email = ? and  BedandBreakfast.hostel_id=Room.hostel_id and Room.room_id = Booking.room_id");
			st.setString(1, user_email);
			rec = st.executeQuery();
			while (rec.next()) {
	...
	public List<Evaluate_hostel> getEvaluateHostel(String hostel_name) throws Exception {
		...
		try {
			conn = makeConnection();
			st = conn.prepareStatement(
					"SELECT Avg(eva_hostel_server)as A,AVG(eva_hostel_location) as B,Avg(eva_hostel_environment) as C, Avg(eva_hostel_food) as D,Avg(eva_hostel_facility)as E FROM Evaluate_hostel WHERE hostel_name = ?");
	...
	public Evaluate_hostel geteva_hostel(ResultSet rec) throws Exception {
		try {
			...
			return new Evaluate_hostel(0, eva_hostel_server, eva_hostel_location, eva_hostel_environment,
					eva_hostel_food, eva_hostel_facility, "", "");
		} catch (Exception e) {
			throw e;
		}
	}
	...
	public List<Evaluate_hostel> getAllEvaluateHostel(String hostel_name) throws Exception {
		...
		try {
			conn = makeConnection();
			st = conn.prepareStatement("SELECT * FROM Evaluate_hostel WHERE hostel_name = ?");
			st.setString(1, hostel_name);
			rec = st.executeQuery();
			while (rec.next()) {
	...
	public Evaluate_hostel getalleva_hostel(ResultSet rec) throws Exception {
		try {
			...
			return new Evaluate_hostel(0, eva_hostel_server, eva_hostel_location, eva_hostel_environment,
					eva_hostel_food, eva_hostel_facility, eva_hostel_feedback, user_email);
		} catch (Exception e) {
			throw e;
		}
	}
